        ...
        <div class="text=center">
        	<ul class="pagination">
        		...
        		<c:if test="${pageMaker.prev}">
        			<li><a href="study/listAll?page=${pageMaker.startPage-1}">&laquo;</a></li>
        		</c:if>
        		
        		<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
        			<li
        			 	<c:out value="${pageMaker.cri.page == idx?'class=active':''}"/>>
        			 	<a href="study/listAll?page=${idx}">${idx}</a>
        		</c:forEach>
        		
        		<c:if test="${pageMaker.next&&pageMaker.endPage>0}">
        			<li><a href="study/listAll?page=${pageMaker.endPage+1}">&raquo;</a></li>
				</c:if>        	
        	</ul>
        </div>
</body>
